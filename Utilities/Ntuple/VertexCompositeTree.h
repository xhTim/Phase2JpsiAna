...
public :
  ...
  Short_t*  charge_mu()                   { SetBranch("charge_mu");                   return charge_mu_;                  }
  ...
  Short_t*  charge_trk()                  { SetBranch("charge_trk");                  return charge_trk_;                 }
  Bool_t*   highPurity_trk()              { SetBranch("HP_trk");                      return highPurity_trk_;             }
 ...
 private:
  ...
  Short_t           charge_mu_[NCAND]={0};   //[candSize]
  ...
  Float_t           pT_trk_[NCAND]={0};   //[candSize]
  Float_t           eta_trk_[NCAND]={0};   //[candSize]
  Float_t           phi_trk_[NCAND]={0};   //[candSize]
  Short_t           charge_trk_[NCAND]={0};   //[candSize]
  Bool_t            highPurity_trk_[NCAND]={0};   //[candSize]
  Float_t           pTErr_trk_[NCAND]={0};   //[candSize]
  Float_t           dXYsig_trk_[NCAND]={0};   //[candSize]
  Float_t           dZsig_trk_[NCAND]={0};   //[candSize]
...
void VertexCompositeTree::InitTree(void)
{
  ...
  if (fChainM_.count("VertexCompositeNtuple")>0) {
    ...
    if (fChain->GetBranch("charge_mu"))                   fChain->SetBranchAddress("charge_mu",                  charge_mu_,                  &(b["charge_mu"])                 );
    ...
    if (fChain->GetBranch("charge_trk"))                  fChain->SetBranchAddress("charge_trk",                 charge_trk_,                 &(b["charge_trk"])                 );
    if (fChain->GetBranch("HP_trk"))                      fChain->SetBranchAddress("HP_trk",                     highPurity_trk_,             &(b["highPurity_trk"])             );
...
void VertexCompositeTree::Clear(void)
{
  ...
  if (GetBranchStatus("charge_mu")==1)        std::fill_n(charge_mu_, nCand_mu, -9.);
  ...
  // CLEAR TRACK INFO VARIABLES
  const auto& nCand_trk = (candSize_trk_>0 ? candSize_trk_ : NCAND);
  if (GetBranchStatus("candSize_trk")==1)      candSize_ = 0;
  ...
  if (GetBranchStatus("charge_trk")==1)        std::fill_n(charge_trk_, nCand_trk, -9.);
  if (GetBranchStatus("HP_trk")==1)            std::fill_n(highPurity_trk_, nCand_trk, 0);
  if (GetBranchStatus("pTErr_trk")==1)         std::fill_n(pTErr_trk_, nCand_trk, -9.);
